...
The application includes \texttt{src/exceptions.py}, featuring specific exceptions like \texttt{FormatError} and \texttt{TimeoutException} for targeted error scenarios. \texttt{main.py} script handles common user errors using try-except blocks.

\subsection{Version Control}
Git was essential for version control in the Sudoku solver's development. Branches were named using feature/ and refactor/ conventions for clarity and stability. A release branch ensured reproducibility for runs in figures \ref{fig:kaggle-million} and \ref{fig:hard-100}. Commit messages described both the reason and the change. A .gitignore file maintained commit relevance.

\subsection{Unit Testing and Continuous Integration}
A test-driven approach guided the development. Initial unit tests remained relevant through ongoing refactoring, using the pytest framework and mocks for sample boards in \texttt{test/\{module\_name\}\_test\_boards}. Pre-commit hooks, defined in \texttt{.pre-commit-config.yaml}, run tests automatically, enforce code style with Black, and check PEP8 compliance with Flake8, aligning with CI/CD practices.

\subsection{Packaging and Accessibility}
For accessibility, the program was containerized using Docker, ensuring lightweight, portable, and reproducible deployments. A Conda environment, reproducible via the environment.yml file, was used. Instructions for running the Docker container are in the README.md.

\section{Conclusion}
This report outlines the Sudoku solver's development, emphasizing problem decomposition, solution design, and implementation, supported by software engineering principles such as modularisation, defensive programming, single responsibility, and encapsulation. These practices ensure an effective solver and foundation for future enhancements.
